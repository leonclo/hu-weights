...
def eclen(cent, rad, v1, v1len, v2, v2len, edgelen):
    if v1len < rad and v2len < rad: return(edgelen)
        ...
        if len(res) == 2: return(np.linalg.norm(res[0]-res[1])) # 2 point intersection length
...
def calculate_edge_weights(tURL, edges, weight_type='uniform', **kwargs):
    ...
    if weight_type == 'uniform':
        ...
        for i in range(n_of_edges):
            ...
            weights[i,0] = edge_length ** alpha #weighted length = edge_length^alpha 
            weights[i,1] = edge_length
    elif weight_type == 'length_binned':
        # 4-bin categorization based on edge length percentiles
        ...
        # Binary short/long classification
        lengths = [np.linalg.norm(tURL[edges[i,0]] - tURL[edges[i,1]]) for i in range(n_of_edges)]
        ...
        # 1/L^beta weighting (emphasizes short edges)
        beta = kwargs.get('beta', 1.0)
        for i in range(n_of_edges):
            edge_length = np.linalg.norm(tURL[edges[i,0]] - tURL[edges[i,1]])
            weights[i,0] = edge_length ** (-beta) #weighted length = 1 / edge_length^beta 
            weights[i,1] = edge_length
...
uselen_w = np.zeros(using.shape[0]) # weighted edge length distance (assign weight based on weight scheme in calculate_edge_weights function)
unweighted = np.zeros(using.shape[0]) #update uselen to weighted scheme
for i in range(using.shape[0]):
    base_length = np.linalg.norm(tURL[using[i,0]]-tURL[using[i,1]])
    if edge_weights is not None:
        uselen[i] = base_length
        uselen_w[i] = base_length * edge_weights[i,2] #same as tovar[k,2] and edge_weights[i,1]
    else:
        uselen[i] = base_length
...
if edge_weights is not None:
    for i in range(Nwind):
        ...
        for j in range(Rs.shape[0]):
            ...
            for k in etc:
                    # EL_IN     TOTAL_EL_WEIGHTED
                    # -----  X  -----------------
                    # TOTAL_EL  TOTAL_EL
                tr = eclen(wind[i], Rs[j], tURL[using[k,0]], verttowind[using[k,0]], tURL[using[k,1]],  verttowind[using[k,1]], uselen[k]) #possibly uselen_w[k] - no, this is right since we want percent inside of the original and then scale based on that
                percent_inside = tr / uselen[k]
                tovar[0,i,j] += (tr) # edge lengths in windows
                tovar[1,i,j] += (percent_inside) # edge lengths in windows #LEON 2
                tovar[2,i,j] += edge_weights[k,2] # edge weights (as a percentage size increase)
else:
    for i in range(Nwind):
        distto = np.zeros(using.shape[0])
        verttowind = np.zeros(tURL.shape[0])
        ...
        for j in range(Rs.shape[0]):
            etc = np.argwhere(distto < Rs[j]).flatten()
            for k in etc:
                tr = eclen(wind[i], Rs[j], tURL[using[k,0]], verttowind[using[k,0]], tURL[using[k,1]],  verttowind[using[k,1]], uselen[k]) # calculating 
                # tr ** 
                tovar[0,i,j] += (tr) #LEON
weighted_edge_density = edge_density(N, tURL, using, uselen_w) # update using edge density-call edge_density with updated using
edge_density_n = edge_density(N, tURL, using, uselen) # update using edge density-call edge_density with updated using
def plot_elv_variance(Rs, tovar, edge_density, weighted_edge_density, N, ctype, ntype, simtag, weight_type):
    ...
    dimensionless_R_w = Rs * (rho_l_w ** 0.5)
    dimensionless_R = Rs * (rho_l_n ** 0.5)
    plt.figure(figsize=(8, 6))
    plt.grid(True, alpha=0.3)
    plt.xlabel(r'$R \rho_\ell^{1/2}$', fontsize=12)
    plt.ylabel('Edge Length Variance', fontsize=12)
    plt.title(f'ELV: {ctype.upper()} N={N} {ntype} wt={weight_type} ρₗ={rho_l_w:.3f}', fontsize=10)
...
end_time = time.time()# End timing computation
